...
bytes constant RUNTIME_CODE =
    hex"5f60143803601491395f5160601c33146016576043565b604036106043576040360360405f375f5f604036035f6020355f355af115603f573d5f5f3e3d5ff35b5f5ffd5b5f5ff3";
...
contract SubAccountFactory is ISubAccountFactory {
    ...
    function deploySubAccount() public returns (address subAccount) {
        ...
        uint256 salt = uint256(uint160(msg.sender) << 96) | accountNumber;
        subAccount = _createSubAccount(salt, msg.sender);
        emit SubAccountDeployed(msg.sender, subAccount, accountNumber);
    }

    /// @inheritdoc ISubAccountFactory
    function getAccount(address user, uint256 accountNumber) public view returns (address) {
    ...
    /// @inheritdoc ISubAccountFactory
    function isAccountDeployed(address user, uint256 accountNumber) public view returns (bool) {
    ...
    function _createSubAccount(uint256 salt, address master) private returns (address subAccount) {
    ...
    function _calculateAddress(address user, uint256 salt) private view returns (address) {
